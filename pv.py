...
# Peak ref power
peakp = np.max(ac_15min)/1000. # kW

# Actual production calculation (extract all available data points)
res = pvlib.iotools.get_pvgis_hourly(latitude,longitude,surface_tilt=latitude,pvcalculation=True,peakpower=peakp,loss=losses)
...
pv = pv['P']
pv = pv.to_frame()
pv = pv.set_index(index60min)

# Resampling at 15 min
pv = pv['P']
pv_15min = pv.resample('15T').pad().reindex(index15min,method='nearest')
